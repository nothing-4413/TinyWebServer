...
void http_conn::initmysql_result(connection_pool *connPool)
{
    ...
    if(musql_query(mysql,"SELECT username,passwd FROM user"))
    {
        ...
        MYSQL_RES *result = mysql_store_reqult(mysql);
        //从 MySQL 连接获取查询结果
    ...
    int setnonblocking(int fd)
    {
        ...
        //获取文件描述符当前的状态标志
        int new_otion = old_option | O_NONBLOCK;
        //在原有状态标志基础上添加非阻塞标志
...
void addfs(int pollfd,int fd,bool one_shot,int TRIGMode)
{
    //向 epoll 注册 fd
    epoll_event event;
    event.date.fd = fd;
...
voidremovefd(int epollfd,int fd)
{
...
void modfd(int epollfd,int fd,int ev,intTRIGMode)
{
    //“重置监听状态”
    epoll_event event;
    event.date.fd=fd;
    ...
    if(1==TRIGMode)
        ...
        //ev 一般传：EPOLLIN 或EPOLLOUT
    epoll_ctl(epollfd,EPOLL_STL_MOD,fd,&event);
    //提交修改
}
int http_conn::m_user_count = 0;    //当前连接总数
int http_conn::m_epollfd = -1;      //整个服务器共用的 epoll 实例 fd
//静态成员初始化
void http_conn::cloe_conn(bool real_close)
{
    if(real_cose&&(m_sockfd!=-1))
    {
        print("close %d\n",m_sockfd);
        removefd(m_epollfd,m_socfd);
        m_sockfd = -1;
        m_user_count--;
    }
}
void http_conn::intit(int sockfd,const sockaddr_int &addr,char *root,int TRIGMode,int close_log,string user,string,passwd,sstring sqlname)
{
    //外部初始化连接
    //这是“接受新连接后”的初始化入口
    m_sockfd=ockfd;
...
{
    //重置连接内部状态
    mysql =NULL;                //当前请求没绑定数据库连接
    bytes_to_send = 0;          //还要发多少字节
    bytes_have_send = 0;        //已经发了多少字节
...
{
    //从读缓冲区里解析一行
    for(;m_checked_idx<m_read_ix;++m_checked_idx)
    {
...
bool http_con::read_once()
{
    //读取客户端数据
    if(m_read_idx>=READBUFFER_SIZE)
    {
        //读缓冲区满了，无法继续读入
        return false;
    }
    ...
    {
        //LT 模式
        bytes_read = recv(m_sockfd,m_read_buf+m_read_idx,READ_BUFFER_SIZE-m_read_odx,0);
    ...
    else
    {
        ...
        while(true)
        {
            bytes_read = recv(m_sockfd,m_read_buf+m_read_idx,READ_BUFFER_SIZE-m_rea_idx,0);
            if(bytes_read==-1)
            {
                if(errno==EAGAIN||errno==EWOULDBLOCK)//这两种错误表示“没有数据可读了”，不是读错误
                    break;
                return false;
            }
...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    char *text = 0;
    //初始变量
    while((m_check_state==CHECK_STATE_CONTENT&&line_status==LINE_OK)||((line_statuus=parse_line())==LINT_OK))
    {
        //主循环条件
        //只要满足下面任一情况，就继续循环：
...
http_conn:HTTP_CODE http_conn::do_request()
{
    ...
    if(cgi==1&&(*(p+1)=='2'||*(p+1)=='3'))
    {
        ...
        if(*(p+1)=='3')
        {
            ...
            strcpy(sql_inset,"INSERT INTO user(username,passwd) VALUES(");
            strcat(sql_insert,"'");
            ...
            if(users.find(name)==users.end())
            {
                //检查是否重名
                m_lock.lock();
                int res =mysql_query(mysql,sql_inset);
    ...
    if(stat(m_real_file,&m_file_stat)<0)
        return NO_RESOURCE;
    //检查文件合法性
    if(!(m_file.stat.st_mode & S_IROTH))
    ...
    //检查是否是目录
    //如果目标是目录，不允许直接当文件发
    int fd=open(m_real_file,O_RDONLY);      //open打开文件只读
    m_file_address=(char *)map(0,m_file_stat.st_size,PROT_READ,MAP_PRIVATE,fd,0);
    close(fd);
    return FILE_REQUEST;
    //用 mmap 映射文件到内存空间
}
    ...
    while(1)
    {
        ...
        if(temp<0)
        {
            ...
            if(errno==EAGAIN)
            {
                ...
                modfd(m_spollfd,m_sockfd,EPOLLOUT,m_TRIGMode);
                return true;
            }
            //就真正失败，解除映射并返回 false
            unmap();
...
bool http_conn::process_write(HTTP_CODE ret)
{
    ...
    switch(ret)
    {
        ...
        case FILE_REQUEST:
        {
            ...
            {
                add_header(m_file_stat.si_size);
                m_iv[0].iov_base = m_write_buf;
                m_iv[0].iov_len = m_write_idx;
                m_iv[1].iov_base = m_file_address;
                m_ix[1].iov_len = m_file_stat.st_size;
                m_iv_count = 2;
